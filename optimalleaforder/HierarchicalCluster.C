...
int 
HierarchicalCluster::cluster(map<int,map<string,int>*>& modules,map<string,HierarchicalClusterNode*>& attribs,double t,map<int,HierarchicalClusterNode*>& attribs_integer,map<string,int>& nameIDMap)
{
	threshold=t;
	globalNodeID=0;
	...
	estimatePairwiseDist(attribs_integer,nameIDMap);
	...
	while(keepMerging && attribs_integer.size()>1)
	{
		...
		//keepMerging=mergePairs_Efficient(attribs_integer);
...
int
HierarchicalCluster::estimatePairwiseDist(map<int,HierarchicalClusterNode*>& currNodeSet, map<string,int>& nameIDMap)
{
	Distance d;
	...
	for(int i=0;i<currNodeSet.size();i++)
	{
		...
		for(int j=i+1;j<currNodeSet.size();j++)
		{
			...
			int attribID=nameIDMap[hcNode2->nodeName];
			//double dist=1-hcNode1->attrib[attribID];
			double dist=hcNode1->attrib[attribID];
			if((strcmp(hcNode1->nodeName.c_str(),"AP4B1")==0) && (strcmp(hcNode2->nodeName.c_str(),"ARAP1")==0))
			{
				cout <<"Stop here" << endl;
			}
			...
			distvalues[i][j]=dist;
			distvalues[j][i]=dist;
			hcNode1->distToNeighbors[j]=dist;
			hcNode2->distToNeighbors[i]=dist;
			...
			myqueue.push(p);
			globalPairID++;
			if(dist<mindusp1 && dusp1)
			{
				//cout <<"curr min with dusp1 " << dist << " " << hcNode1->nodeName <<" " << hcNode2->nodeName << endl;
				mindusp1=dist;
			}
		}
	}
